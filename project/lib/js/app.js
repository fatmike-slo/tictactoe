...
$(document).ready(() => {
    ...
    $("#c2").on("click", () => {
        processPlayerInput(7);
    });
    $("#c3").on("click", () => {
        processPlayerInput(8);
    });
    // info button
    $("#infoButton").on("click", () => {
    ...
    // process user events ("clicks")
    function processPlayerInput(index) {
        if (turnX === true && endGame === false) {
            if (dataObj[index].checked === false) {
                dataObj[index].owner = "X";
                dataObj[index].display();
                dataObj[index].checked = true;
                pPosArr.push(dataObj[index].id);
    ...
    function aiMove() {
        ...
        // --------------- third and on move -----------------
            ...
            if (nextMove === false) {
                console.log('third move: nextMove je false, postavi random!');
                generateSecondMove();
                checkVictory();
                nextTurn();
            }
            else if (nextMove === true) {
                console.log('third move: al pa tle, ne naredit nic');
                nextTurn();
                nextMove = false;
            }
